...
import { getAwsCost, verifyAwsCredentials } from "../services/aws.service.js";
...
export const getCost = async (req, res) => {
  try {
    const org = await Organization.findById(req.user.orgId).select("+awsAccessKey +awsSecretKey");

   

    if (!org.awsAccessKey || !org.awsSecretKey || !org.awsRegion) {
      return res.status(400).json({ message: "AWS credentials not configured. Please connect AWS first." });
    }
