...
const makeCW = (ak, sk, region) =>
  new CloudWatchClient({ region, ...creds(ak, sk) });
...
const makeDDB = (ak, sk, region) =>
  new DynamoDBClient({ region, ...creds(ak, sk) });
...
const fetchLatestDatapoint = async (
  cwClient,
  { namespace, metricName, dimensions, windowMinutes = 70 },
) => {
  const now = new Date();
...
export const fetchRDSInstances = async (ak, sk, region) => {
  ...
  const res = await rds.send(
    new DescribeDBInstancesCommand({ MaxRecords: 100 }),
  );
